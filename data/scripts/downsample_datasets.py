...
@click.option(
    ...
    required=True,
    help='Directory containing input CSV and images'
)
@click.option(
    '--csv-file', '-c',
    default='processed.csv',
    show_default=True,
    help='Name of the CSV file to read and update'
)
@click.option(
    '--size', '-s',
    default='224x224',
    help='Max dimensions in WIDTHxHEIGHT format'
)
@click.option(
    '--quality', '-q',
    default=85,
...
def main(dir_path, csv_file, size, quality, workers):
    dir_path = Path(dir_path)
    proc_csv = dir_path / csv_file
    ...
    try:
        max_w, max_h = map(int, size.lower().split('x'))
    except ValueError:
